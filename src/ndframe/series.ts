...
import { ArrayType1D } from '../utils/arrays';
import NDframe from './ndframe';

type mapFunc = (val: any, i: number) => any[]

type mapParam = object | mapFunc
...
export default class Series extends NDframe {
  maxRow: number
  constructor(data: any = [], options: /*BaseDataOptionType*/ any = {}) {
    const { index, columns, dtypes, config } = options;
    if (Array.isArray(data[0]) || NDframe.utils.isObject(data[0])) {
      data = NDframe.utils.convert2DArrayToSeriesArray(data);
      super({
        data,
        index,
        columns,
  ...
  iloc(rows: Array<string | number | boolean>): Series {
    ...
    const _columnIndexes = range(0, this.shape[1] - 1)
    const newData = []
    const newIndex = []
  ...
  loc(rows: Array<string | number | boolean>): Series {
    ...
    const _columnNames = this.columns

    const _columnIndexes = _columnNames.map(columnName => _columnNames.indexOf(columnName))// Return all column index
  ...
  add(other: Series | Array<number> | number, options?: { inplace?: boolean }): Series | void {
    ...
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support add operation`);
    }
    let newData;
    if (typeof other === "number") {
      newData = (this.values as number[]).map((ele => ele + other))
    } else if (other instanceof Series) {
      this.checkSeriesOpCompatibility(other, 'add');
      newData = (this.values as number[]).map((ele, index) => { return ele + (other.values as number[])[index] })
    } else if (Array.isArray(other)) {
      newData = (this.values as number[]).map((ele, index) => { return ele + (other as number[])[index] })
    } else {
    ...
    if (inplace) {
      ...
      return this.createNdframeFromNewDataWithOldProps({ newData })
    }
  }
  ...
  sub(other: Series | number | Array<number>, options?: { inplace?: boolean }): Series | void {
    ...
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support sub operation`);
    }
    ...
    if (typeof other === "number") {
      ...
      this.checkSeriesOpCompatibility(other, 'sub');
      newData = (this.values as number[]).map((ele, index) => { return ele - (other.values as number[])[index] })
    } else if (Array.isArray(other)) {
      newData = (this.values as number[]).map((ele, index) => { return ele - (other as number[])[index] })
    } else {
  ...
  mul(other: Series | number | Array<number>, options?: { inplace?: boolean }): Series | void {
    ...
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support mul operation`);
    }
    ...
    if (typeof other === "number") {
      ...
      this.checkSeriesOpCompatibility(other, 'mul');
      newData = (this.values as number[]).map((ele, index) => { return ele * (other.values as number[])[index] })
    } else if (Array.isArray(other)) {
      newData = (this.values as number[]).map((ele, index) => { return ele * (other as number[])[index] })
    } else {
  ...
  div(other: Series | number | Array<number>, options?: { inplace?: boolean }): Series | void {
    ...
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support div operation`);
    }
    ...
    if (typeof other === "number") {
      ...
      this.checkSeriesOpCompatibility(other, 'div');
      newData = (this.values as number[]).map((ele, index) => { return ele / (other.values as number[])[index] })
    } else if (Array.isArray(other)) {
      newData = (this.values as number[]).map((ele, index) => { return ele / (other as number[])[index] })
    } else {
  ...
  pow(other: Series | number | Array<number>, options?: { inplace?: boolean }): Series | void {
    ...
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support pow operation`);
    }
    ...
    if (typeof other === "number") {
      ...
      this.checkSeriesOpCompatibility(other, 'pow');
      newData = (this.values as number[]).map((ele, index) => { return ele ** (other.values as number[])[index] })
    } else if (Array.isArray(other)) {
      newData = (this.values as number[]).map((ele, index) => { return ele ** (other as number[])[index] })
    } else {
  ...
  mod(other: Series | number | Array<number>, options?: { inplace?: boolean }): Series | void {
    ...
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support mod operation`);
    }
    ...
    if (typeof other === "number") {
      ...
      this.checkSeriesOpCompatibility(other, 'mod');
      newData = (this.values as number[]).map((ele, index) => { return ele % (other.values as number[])[index] })
    } else if (Array.isArray(other)) {
      newData = (this.values as number[]).map((ele, index) => { return ele % (other as number[])[index] })
    } else {
  ...
  private $checkAndCleanValues(values: ArrayType1D, operation: string): number[] {
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support ${operation} operation`);
    }
    values = NDframe.utils.removeMissingValuesFromArray(values);

    if (this.dtypes[0] == "boolean") {
      values = (NDframe.utils.mapBooleansToIntegers(values as boolean[], 1) as ArrayType1D);
    }
   ...
   */
  count(): number {
    const values = NDframe.utils.removeMissingValuesFromArray(this.values as ArrayType1D)
  ...
  maximum(other: Series | number | Array<number>): Series {
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support maximum operation`);
    }

    let newData;
    if (typeof other === "number") {
      newData = (this.values as number[]).map((ele => Math.max(ele, other)))
    } else if (other instanceof Series) {
      this.checkSeriesOpCompatibility(other, 'maximum');
      newData = (this.values as number[]).map((ele, index) => { return Math.max(ele, (other.values as number[])[index]) })
    } else if (Array.isArray(other)) {
      newData = (this.values as number[]).map((ele, index) => { return Math.max(ele, (other as number[])[index]) })
    } else {
      throw new Error("ParamError: value for other not supported. It must be either a scalar, Array or Series");
    }
  ...
  minimum(other: Series | number | Array<number>): Series {
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support minimum operation`);
    }

    let newData;
    if (typeof other === "number") {
      newData = (this.values as number[]).map((ele => Math.min(ele, other)))
    } else if (other instanceof Series) {
      this.checkSeriesOpCompatibility(other, 'maximum');
      newData = (this.values as number[]).map((ele, index) => { return Math.min(ele, (other.values as number[])[index]) })
    } else if (Array.isArray(other)) {
      newData = (this.values as number[]).map((ele, index) => { return Math.min(ele, (other as number[])[index]) })
    } else {
      throw new Error("ParamError: value for other not supported. It must be either a scalar, Array or Series");
    }
    return new Series(newData, {
  ...
  round(dp = 1, options?: { inplace?: boolean }): Series | void {
    ...
    const newValues = NDframe.utils.round(this.values as number[], dp, true);

    if (inplace) {
      this.$setValues(newValues)
    } else {
      return this.createNdframeFromNewDataWithOldProps({ newData: newValues })
    }
  }
  ...
  isNa(): Series {
    const newData = this.values.map((value) => {
      ...
      if (NDframe.utils.isEmpty(value)) {
        return true;
      } else {
        return false;
      }
  ...
  sortValues(options?: { ascending?: boolean, inplace?: boolean }): Series | void {
    ...
    let sortedValues = [];
    let sortedIndex = []
    const rangeIdx = NDframe.utils.range(0, this.index.length - 1);
    const sortedIdx = NDframe.utils.sortArrayByIndex(rangeIdx, this.values, this.dtypes[0]);

    for (const indx of sortedIdx) {
  ...
  unique(): Series {
    ...
    const series = new Series(Array.from(newValues));
    return series;
  }
  ...
  abs(options?: { inplace?: boolean }): Series | void {
    ...
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support abs operation`);
    }

    const newValues = this.values.map(val => Math.abs(val as number));
  ...
  private cumOps(ops: string, options: { inplace: boolean }): Series | void {
    if (this.dtypes[0] == "string") {
      throw new Error(`DtypeError: String data type does not support ${ops} operation`);
    }
  ...
  private boolOps(other: Series | number | Array<number> | boolean[], bOps: string) {
    ...
    for (let i = 0; i < lSeries.length; i++) {
      const lVal = lSeries[i];
      const rVal = rSeries[i];
      let bool = null;
      switch (bOps) {
        case "lt":
  ...
  asType(dtype: "float32" | "int32" | "string" | "boolean" | "undefined", options?: { inplace?: boolean }): Series | void {
    ...
    if (!(NDframe.DATA_TYPES.includes(dtype))) {
      throw Error(`dtype ${dtype} not supported. dtype must be one of ${NDframe.DATA_TYPES}`);
    }
   ...
   */
  createNdframeFromNewDataWithOldProps({ newData }: { newData: any }) {
    return new Series(
      newData,
      {
        index: [...this.index],
  ...
  checkSeriesOpCompatibility(other: Series, operation: string): void {
    if (this.shape[0] != other.shape[0]) {
      const msg = `ParamError: Row length mismatch. Length of other (${other.shape[0]}), must be the same as Ndframe (${this.shape[0]})`
      throw new Error(msg)
    }
    if (this.dtypes[0] == 'string' || other.dtypes[0] == 'string') {
